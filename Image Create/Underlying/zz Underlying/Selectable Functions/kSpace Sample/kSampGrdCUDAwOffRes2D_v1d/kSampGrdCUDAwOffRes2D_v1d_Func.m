...
function [KSMP,err] = kSampGrdCUDAwOffRes2D_v1d_Func(KSMP,INPUT)
...
%---------------------------------------------
Type = 'M2M';
[Ksz,SubSamp,Kx,Ky,KERN,CONV,err] = ConvSetupTest2D_v1a(IMP,KRNprms,Type);
centre = (Ksz+1)/2;
...
for n = 1:OB.ObMatSz
    for m = 1:OB.ObMatSz
        rad = sqrt((n-centre)^2 + (m-centre)^2);
        if rad > PROJdgn.rad;
            K(n,m) = 0;
        end
    end
...
%---------------------------------------------
zfOb = zeros(ZF,ZF);
zfOb(Imbot:Imtop,Imbot:Imtop) = Ob;
zfOb = zfOb./IFprms.V;
zfOb = ifftshift(zfOb);
...
Off = zeros(ZF,ZF);
Off(Imbot:Imtop,Imbot:Imtop) = B0Map;
Off = ifftshift(Off);
...
StatLev = 3;
[Dat,err] = mSampOffResCUDADoubles2D_v1f(zfOb,Off,T,Kx,Ky,KERN,CONV,StatLev);
if err.flag
    return
end
