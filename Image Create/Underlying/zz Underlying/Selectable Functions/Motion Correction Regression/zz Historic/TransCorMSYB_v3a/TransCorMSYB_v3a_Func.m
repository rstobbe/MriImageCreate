...
function [TRANSCOR,err] = TransCorMSYB_v3a_Func(TRANSCOR,INPUT)
...
if subsamp ~= 1
    err.flag = 1;
    err.msg = 'TransCorMSYB_v2a only designed for SubSamp = 1';
end
...
for n = 1:nproj
    ...
    elseif n > 1
        ...
        %---------------------------------------------
        % Phase Ramp X
        Xphdif0 = squeeze(phdif(2,:,2));
        [r,phslp(n,2),pfoffset] = regression(T,Xphdif,'one');
        %plot(t,t*phslp(n,2)+pfoffset,'r');
        
        %---------------------------------------------
        % Phase Ramp Y
        Yphdif0 = permute(squeeze(phdif(:,2,2)),[2,1]);
        [r,phslp(n,1),pfoffset] = regression(T,Yphdif,'one');

        %---------------------------------------------
        % Phase Ramp Z
        Zphdif0 = permute(squeeze(phdif(2,2,:)),[2,1]); 
        [r,phslp(n,3),pfoffset] = regression(T,Zphdif,'one');
