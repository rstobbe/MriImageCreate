...
function [GRD,err] = GridkSpace_LclKernS_v1k_Func(GRD,INPUT)
...
if NumberGpus == 2
    if ComputeCapability > 6.0
        ...
        elseif strcmp(GRD.type,'complex')
            %[GrdDat,err] = mS2GCUDASingleC_v5b(ZF,Kx,Ky,Kz,KERN,SampDat,CONV,StatLev,CUDA);
            [GrdDat,err] = mS2GCUDASingleC_v5c(ZF,Kx,Ky,Kz,KERN,SampDat,CONV,StatLev,CUDA);         % should be identical to v5b
        end
    else
        err.flag = 1;
