...
function [DCCOR,err] = DCcorPA_TrlFid_v1d_Func(DCCOR,INPUT)
...
elseif strcmp(DCCOR.type,'Individual')
    for n = 1:nrcvrs
        for m = 1:nproj
            stetest(n,m) = max(abs(FIDmat(m,np-trfidpts+1:np,n)))/mean(abs(FIDmat(m,np-trfidpts+1:np,n)));
            if stetest(n,m) > 2
                ...
                xlabel('Readout Points'); title('DC Offset (One Trajectory)');
                err.flag = 1;
                err.msg = 'Stimulated echo likely causing DC correction error';
                return
            end
