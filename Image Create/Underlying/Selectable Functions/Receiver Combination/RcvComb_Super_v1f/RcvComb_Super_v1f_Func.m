...
function [RCOMB,err] = RcvComb_Super_v1f_Func(RCOMB,INPUT)
...
if strcmp(vis,'Yes')
    ...
    IMSTRCT.rows = 5; IMSTRCT.lvl = [0 max(abs(lowresIms(:)))]; IMSTRCT.SLab = 0; 
    IMSTRCT.docolor = 0; IMSTRCT.figsize = [];  
    for n = 1:rcvrs
        IMSTRCT.figno = 5000+n; 
        ImageMontage_v2a(lowresIms(:,:,:,n),IMSTRCT);
    end
end
...
% Return
%-------------------------------------------
RCOMB.Im = (sum((highresIms.*conj(lowresIms)),4))./sqrt(sum((lowresIms.*conj(lowresIms)),4));
