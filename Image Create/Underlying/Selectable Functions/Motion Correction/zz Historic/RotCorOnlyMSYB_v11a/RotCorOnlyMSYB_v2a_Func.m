...
function [MOTCOR,err] = RotCorOnlyMSYB_v2a_Func(MOTCOR,INPUT)
...
for n = 1:nproj
    %---------------------------------------------
    % Isolate Centre
    rad = (sqrt(Kmat(n,:,1).^2 + Kmat(n,:,2).^2 + Kmat(n,:,3).^2))/kstep;
    subnpro = find(rad >= (trajmax),1,'first');
    subkmax = rad(subnpro-1)*kstep;
    subKmat = Kmat(n,1:subnpro,:);
    subDAT = squeeze(DAT(n,1:subnpro));
